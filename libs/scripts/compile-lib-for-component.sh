...
rm -rf ${DIR}/libs/${MODULE}

app_dependencies=( $(npm ls --omit=dev --all --json | ${THISSCRIPTDIR}/list-dependencies.js) )
...
lib_dependencies=( $(npm ls --omit=dev --json | ${THISSCRIPTDIR}/list-dependencies.js) )
...
echo "libs/${MODULE}: tsc -p $tsconfig_file --declaration --outDir ${DIR}/libs/${MODULE}..."
./node_modules/.bin/tsc -p $tsconfig_file --declaration --outDir ${DIR}/libs/${MODULE}
