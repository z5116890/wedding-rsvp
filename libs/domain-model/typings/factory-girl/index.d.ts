// @ts-ignore suppress error if `chance` is not installed because not every component uses `factory-girl` and `chance`
import { Chance } from 'chance'

type ChanceMethods = Omit<Chance.Chance, 'seed'> // ignore `seed` as it's not a chance method
...
declare namespace FactoryGirl {
  ...
  class FactoryGirl {
     ...
     */
    chance<T extends keyof ChanceMethods>(chanceMethod: T, opts?: Parameters<ChanceMethods[T]>[0]):
      () => ReturnType<ChanceMethods[T]>
...
export type FactoryGirl = FactoryGirl.FactoryGirl
